 ...
 */
class ClearCachePlugins extends AbstractTask
{
    /**
     * @var int
     */
    ...
    public function execute()
    {
        ...
        // Clear cache for plugins
        $resultArray = [];

        // Get pages with plugins
        $pagesArray = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('pages.uid', 'pages, tt_content', $this->getWhereClause(), 'uid');
        foreach ($pagesArray as $page) {
            $resultArray = array_merge_recursive(
                $resultArray,
                $this->communicationService->sendClearCacheCommandForTables('pages', $page['uid'], $this->tx_vcc_hosts)
            );
        }
        unset($page);
    ...
    {
        $whereClauseArray = [];
        $whereClauseArray[] = 'tt_content.pid=pages.uid';
        $whereClauseArray[] = 'FIND_IN_SET(tt_content.list_type,' .
            $GLOBALS['TYPO3_DB']->fullQuoteStr(implode(',', $this->tx_vcc_plugins), 'tt_content') . ')';
        ...
        unset($page);
        if (!empty($pidListArray)) {
            // Send debug administration mail with pages information
            $this->sendAdministrationMail('pidListArray: ' . implode(',', $pidListArray));
            $whereClauseArray[] = 'pages.uid IN (' . implode(',', $pidListArray) . ')';
        }

        if ($this->tx_vcc_access) {
            $whereClauseArray[] = 'pages.hidden=0';
        }

        if (empty($this->tx_vcc_deleted)) {
            $whereClauseArray[] = 'pages.deleted=0';
        }
    ...
    /**
     * @param array $pidListArray
     * @param int $id
     * @param int $depth
     * @param int $checkAccess
    ...
    protected function sendAdministrationMail($message)
    {
        if (!empty($this->tx_vcc_email)) {
            ...
            $mail->setSubject('Administration information for \'Clear Varnish cache for plugins\' task #' . $this->getTaskUid());
